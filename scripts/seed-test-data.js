 ...
 *
 * Usage:
 *   node scripts/seed-test-data.js seed     - Seed the database
 *   node scripts/seed-test-data.js cleanup  - Remove test data
 */
 ...
 */
function createSilentMP3(filePath, durationSeconds = 3) {
...
function seedDatabase() {
  ...
  if (!fs.existsSync(DB_PATH)) {
    console.error('[SeedTestData] ERROR: Database not found at', DB_PATH);
    console.error('[SeedTestData] Run migrations first: sqlx migrate run --source libraries/soul-storage/migrations');
    process.exit(1);
  }
...
if (command === 'seed') {
  ...
  console.log('Usage: node scripts/seed-test-data.js [seed|cleanup]');
  process.exit(1);
}
