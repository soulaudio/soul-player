...
fn test_dirac_impulse_passes_signal_through_short_ir() {
    ...
    println!(
        "Dirac IR (1 sample) null test difference: {:.2} dB",
        diff_db
    );
...
fn test_dirac_impulse_with_various_signal_types() {
    ...
    let impulse_diff_db = linear_to_db(calculate_difference_rms(
        &impulse_original,
        &impulse_processed,
    ));
    ...
    assert!(
        dc_diff_db < -40.0,
        "DC signal should pass through. Diff: {:.2} dB",
        dc_diff_db
    );
    assert!(
        hf_diff_db < -40.0,
        "HF signal should pass through. Diff: {:.2} dB",
        hf_diff_db
    );
    assert!(
        impulse_diff_db < -40.0,
        "Impulse should pass through. Diff: {:.2} dB",
        impulse_diff_db
    );
}
...
fn test_short_ir_time_domain_processing() {
    ...
    assert!(
        buffer[0].abs() > 0.01,
        "First output sample should be significant"
    );
    ...
    println!(
        "Short IR (32 samples): peak={:.3}, early_energy={:.3}, late_energy={:.3}",
    ...
    assert!(
        ...
        "Signal should decay: early={:.3}, late={:.3}",
        early_energy,
        late_energy
    );
}
...
fn test_medium_ir_fft_processing() {
    ...
    println!(
        "Medium IR (256 samples): early={:.4}, mid={:.4}, late={:.6}",
...
fn test_long_ir_processing() {
    ...
    println!(
        "Long IR: length=2000, energy={:.2}, RMS={:.4}",
        ir_energy, ir_rms
    );
    ...
    println!(
        "Long IR (2000 samples): peak={:.3}, RMS={:.4}, gain={:.2}x",
...
fn test_very_short_ir_edge_case() {
    ...
    for ir_length in [1, 2, 4] {
        ...
        assert!(
            buffer.iter().all(|x| x.is_finite()),
            "IR length {} should produce finite output",
            ir_length
        );
    }
}
...
fn test_dry_wet_mix_zero_is_fully_dry() {
    ...
    assert!(
        diff_db < -60.0,
        "Fully dry should pass signal unchanged. Diff: {:.2} dB",
        diff_db
    );
}
...
fn test_dry_wet_mix_one_is_fully_wet() {
    ...
    println!(
        "Dry/wet 1.0 - difference from original: RMS={:.4}",
        diff_rms
    );
    ...
    assert!(
        diff_rms > 0.01,
        "Fully wet with non-unity IR should differ from dry. Diff RMS: {:.4}",
        diff_rms
    );
}
...
fn test_dry_wet_mix_fifty_percent() {
    ...
    assert!(
        diff_db < -40.0,
        "50% mix with Dirac should equal input. Diff: {:.2} dB",
        diff_db
    );
}
...
fn test_dry_wet_mix_clamping() {
    ...
    assert_eq!(
        engine.dry_wet_mix(),
        0.0,
        "Negative mix should clamp to 0.0"
    );
    ...
    assert!(
        (engine.dry_wet_mix() - 0.75).abs() < 0.001,
        "Valid mix should be stored"
    );
}
...
fn test_fft_time_domain_boundary() {
    ...
    engine_time
        .load_impulse_response(&ir_short, 44100, 2)
        .unwrap();
    ...
    engine_fft
        .load_impulse_response(&ir_long, 44100, 2)
        .unwrap();
    ...
    assert!(
        ratio > 0.5 && ratio < 2.0,
        "Time-domain and FFT should produce similar energy. Ratio: {:.2}",
        ratio
    );
}
...
fn test_continuous_processing_no_growth() {
    ...
    for iteration in 0..1000 {
        ...
        assert!(
            buffer.iter().all(|x| x.is_finite()),
            "Output should be finite at iteration {}",
            iteration
        );
        ...
        assert!(
            peak < 100.0,
            "Peak shouldn't explode. Iteration {}: peak={}",
            iteration,
            peak
        );
    }
...
fn test_stereo_ir_independent_channels() {
    ...
    assert!(
        left_samples[0].abs() > 0.1,
        "Left channel should have immediate response at frame 0: got {:.3}",
        left_samples[0]
    );
...
fn test_mono_ir_applied_to_both_channels() {
    ...
    assert!(
        diff_db < -30.0,
        "Mono Dirac IR should pass stereo signal through. Diff: {:.2} dB",
        diff_db
    );
}
...
fn test_energy_preservation_with_unity_ir() {
    ...
    println!(
        "Energy preservation: ratio={:.4}, diff={:.2} dB",
        energy_ratio, energy_diff_db
    );

    // Energy should be approximately preserved (within 1 dB)
    assert!(
        energy_diff_db.abs() < 1.0,
        "Energy should be preserved with Dirac IR. Diff: {:.2} dB",
        energy_diff_db
    );
}
...
fn test_energy_scaling_with_attenuating_ir() {
    ...
    println!(
        "Energy with 0.5 IR: expected={:.4}, actual={:.4}",
    ...
    assert!(
        ...
        "Energy should scale with IR^2. Expected ~{:.2}, got {:.2}",
        expected_ratio,
        actual_ratio
    );
}
...
fn test_output_appears_at_expected_time() {
    ...
    println!(
        "Input impulse at frame 100, output first significant at: {:?}",
    ...
    if let Some(pos) = first_significant {
        ...
        assert!(
            frame_pos <= 600,
            "Output should appear within reasonable time. Got frame {}",
            frame_pos
        );
    }
}
...
fn test_null_test_with_dirac_ir() {
    ...
    let null_signal: Vec<f32> = original
        .iter()
        .zip(processed.iter())
        .map(|(a, b)| a - b)
        .collect();
    ...
    println!(
        "Null test: residual={:.2} dB, SNR={:.1} dB",
        null_db, snr_db
    );
    ...
    assert!(
        null_db < -40.0 || snr_db > 40.0,
        "Null test should show high cancellation. Residual: {:.2} dB",
        null_db
    );
}
...
fn test_disabled_engine_passes_through() {
    ...
    assert!(
        diff_db < -100.0 || processed == original,
...
fn test_reset_clears_state() {
    ...
    assert!(
        silence_db < -20.0 || silence_peak < 0.1,
        "Reset should clear convolution tail. Peak: {:.4}",
        silence_peak
    );
}
...
fn test_sample_rate_metadata() {
    ...
    assert!(
        ...
        "Duration should be {:.4}s, got {:.4}s",
        expected_duration,
        duration
    );
}
