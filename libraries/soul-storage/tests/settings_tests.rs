...
async fn test_get_all_settings() {
    ...
    settings::set_setting(&pool, "1", settings::SETTING_THEME, &serde_json::json!("dark"))
        .await
        .unwrap();
    settings::set_setting(&pool, "1", settings::SETTING_LOCALE, &serde_json::json!("en-US"))
        .await
        .unwrap();
    ...
    assert!(all_settings.iter().any(|s| s.key == settings::SETTING_THEME));
    assert!(all_settings.iter().any(|s| s.key == settings::SETTING_LOCALE));
    assert!(all_settings.iter().any(|s| s.key == settings::SETTING_VOLUME));
}
...
async fn test_multi_user_isolation() {
    ...
    settings::set_setting(&pool, "1", settings::SETTING_THEME, &serde_json::json!("light"))
        .await
        .unwrap();
    settings::set_setting(&pool, "2", settings::SETTING_THEME, &serde_json::json!("dark"))
        .await
        .unwrap();
...
async fn test_json_value_types() {
    ...
    settings::set_setting(&pool, "1", "key_string", &string_val).await.unwrap();
    settings::set_setting(&pool, "1", "key_number", &number_val).await.unwrap();
    settings::set_setting(&pool, "1", "key_bool", &bool_val).await.unwrap();
    settings::set_setting(&pool, "1", "key_object", &object_val).await.unwrap();
    settings::set_setting(&pool, "1", "key_array", &array_val).await.unwrap();

    assert_eq!(settings::get_setting(&pool, "1", "key_string").await.unwrap(), Some(string_val));
    assert_eq!(settings::get_setting(&pool, "1", "key_number").await.unwrap(), Some(number_val));
    assert_eq!(settings::get_setting(&pool, "1", "key_bool").await.unwrap(), Some(bool_val));
    assert_eq!(settings::get_setting(&pool, "1", "key_object").await.unwrap(), Some(object_val));
    assert_eq!(settings::get_setting(&pool, "1", "key_array").await.unwrap(), Some(array_val));
}
