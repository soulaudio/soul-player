...
use crate::{sources, tracks, artists, albums, playlists};
...
impl StorageContext for LocalStorageContext {
    ...
    async fn get_user_playlists(&self) -> Result<Vec<Playlist>> {
        playlists::get_user_playlists(&self.pool, self.user_id).await
    }
    
    async fn get_playlist_by_id(&self, id: PlaylistId) -> Result<Option<Playlist>> {
        playlists::get_by_id(&self.pool, id, self.user_id).await
    }
    
    async fn get_playlist_with_tracks(&self, id: PlaylistId) -> Result<Option<Playlist>> {
        playlists::get_with_tracks(&self.pool, id, self.user_id).await
    }
    ...
    async fn add_track_to_playlist(&self, playlist_id: PlaylistId, track_id: TrackId) -> Result<()> {
        playlists::add_track(&self.pool, playlist_id, track_id, self.user_id).await
    }
    
    async fn remove_track_from_playlist(&self, playlist_id: PlaylistId, track_id: TrackId) -> Result<()> {
        playlists::remove_track(&self.pool, playlist_id, track_id, self.user_id).await
    }
    
    async fn delete_playlist(&self, id: PlaylistId) -> Result<()> {
        playlists::delete(&self.pool, id, self.user_id).await
    }

    // Play History & Stats
    async fn record_play(&self, track_id: TrackId, duration_seconds: Option<f64>, completed: bool) -> Result<()> {
        tracks::record_play(&self.pool, self.user_id, track_id, duration_seconds, completed).await
    }
    
    async fn get_recently_played(&self, limit: i32) -> Result<Vec<Track>> {
        tracks::get_recently_played(&self.pool, self.user_id, limit).await
    }
